    ...
    <div className="col-9 bg-white px-4 detail__box">
      <div className="d-flex justify-content-between">
        <p className="font-weight-bold mt-3">
          Known for <span className="text-muted small">({showMovies ? "Movies" : "TV"})</span>
        </p>
        ...
        {person.id &&
          displayItems.map(item => {
            return (
              <Link
                key={item.credit_id}
                className="col-6 col-lg-3 text-decoration-none text-reset poster__link"
              ...
              >
                <img
                  ...
                  className={`img-fluid poster__img`}
                />
                <p className="text-center  p-2 font-weight-light">{item.title || item.name}</p>
              </Link>
            );
          })}
      </div>
